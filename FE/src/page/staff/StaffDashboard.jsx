  ...
  export default function StaffDashboard({ user, userRole }) {
    ...
    const getStatusBadge = (status) => {
      ...
      switch (statusText) {
        ...
        case 'cancelled': label = 'Đã hủy'; className = 'status-declined'; break; // Thêm trạng thái hủy
      }

      return <span className={`status-badge ${className}`}>{label}</span>;
    };

    // Hàm kiểm tra các trạng thái mà công việc đã bắt đầu/có Checklist
    const hasChecklist = (status) => {
      const statusText = status ? status.toLowerCase() : '';
      return ['completed', 'paid'].includes(statusText);
    };
                    ...
                    filteredAppointments.map((appt) => {
                      ...
                      return (
                        <tr key={appt.bookingId}>
                          ...
                          <td>
                            {isPending ? (
                              // TRẠNG THÁI CHỜ XỬ LÝ: DUYỆT / TỪ CHỐI
                              ...
                              // TRẠNG THÁI ĐÃ DUYỆT & CHƯA PHÂN CÔNG: PHÂN CÔNG
                              <div className="action-buttons-cell">
                                <button
                            ...
                            ) : hasChecklist(appt.status) ? (
                              // CÁC TRẠNG THÁI ĐÃ CÓ CHECKLIST (assigned, in progress, completed, paid): XEM
                              <button
                              ...
                              // TRẠNG THÁI CÒN LẠI (Declined, Cancelled...)
