...
export default function FeedbackGaugeChart({ feedback }) {
    ...
    return (
        <div style={{ width: '100%', maxWidth: 360, margin: '0 auto', height: 220, position: 'relative' }}>
            ...
            <div style={{
                position: 'absolute', top: '55%', left: '50%', transform: 'translate(-50%, -50%)', textAlign: 'center'
            }}>
                <div style={{ fontSize: '2.4rem', fontWeight: 800, color: '#f59e0b' }}>{avg.toFixed(1)}</div>
                <div style={{ fontSize: '0.95rem', color: '#6b7280' }}>/5 ({feedback?.totalFeedbacks ?? 0} lượt)</div>
            </div>
        </div>
    );
}
