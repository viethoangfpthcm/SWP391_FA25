...
export default function AddVehicleModal({
  ...
  const minPurchaseDate = newVehicleData.year ? `${newVehicleData.year}-01-01` : undefined;

  const handleSubmit = (e) => {
    e.preventDefault();
    if (newVehicleData.year && newVehicleData.purchaseDate) {
      const mfgYear = parseInt(newVehicleData.year);
      const purchaseYear = new Date(newVehicleData.purchaseDate).getFullYear();

      if (purchaseYear < mfgYear) {
        alert(`Ngày mua không hợp lệ! Xe sản xuất năm ${mfgYear} không thể mua trước thời điểm đó.`);
        return; 
      }
    }
    onSave(e);
